import { conversations, leads, queueItems, usageStats } from "@/lib/mock-data";
import { formatMoney } from "@/lib/utils";
import { StatCard } from "@/components/ui/stat-card";

export function OverviewGrid() {
  const dueQueue = queueItems.filter((item) => item.status === "pending").length;
      ...
      <div className="grid gap-4 xl:grid-cols-[1.4fr_0.8fr]">
        <div className="card glow p-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Multi-workspace pipeline radar</h2>
            <span className="text-sm text-slate-400">Last 7 days</span>
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-4">
            {[
              { label: "Open deals", value: "48" },
              { label: "Queued follow-ups", value: String(dueQueue) },
        ...
        <div className="space-y-4">
          ...
          <div className="card glow p-6">
            <h2 className="text-lg font-semibold">Queue execution base</h2>
            <p className="mt-3 text-sm text-slate-400">
              Cron-ready endpoint available at <span className="text-slate-200">/api/queue/process</span>.
            </p>
            <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
              <p>Webhook inbound: WhatsApp + Meta channels</p>
              <p className="mt-2">Persistence scope: leads, queue, integrations and subscriptions per workspace</p>
            </div>
          </div>
        </div>
      </div>
